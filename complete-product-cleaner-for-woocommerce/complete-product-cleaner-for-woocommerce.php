...
add_action('admin_init', 'compprcl_check_woocommerce');
function compprcl_check_woocommerce()
{
...
// Add admin menu
add_action('admin_menu', 'compprcl_add_admin_menu');
function compprcl_add_admin_menu()
{
    add_submenu_page(
        'woocommerce',
        'Product Cleaner',
...
add_action('admin_enqueue_scripts', 'compprcl_enqueue_admin_assets');
function compprcl_enqueue_admin_assets($hook)
{
    if ($hook != 'woocommerce_page_complete-product-cleaner-for-woocommerce') {
        return;
    }

    wp_enqueue_style('compprcl-admin-css', plugin_dir_url(__FILE__) . 'css/complete-product-cleaner-for-woocommerce.css', array(), '1.0.5');
    wp_enqueue_script('compprcl-admin-js', plugin_dir_url(__FILE__) . 'js/complete-product-cleaner-for-woocommerce.js', array('jquery'), '1.0.5', true);

    wp_localize_script('compprcl-admin-js', 'compprclData', array(
        'ajaxUrl' => admin_url('admin-ajax.php'),
        'nonce' => wp_create_nonce('compprcl_cleanup_nonce'),
        'placeholder' => function_exists('wc_placeholder_img_src') ? wc_placeholder_img_src() : ''
    ));
}

// AJAX: Get product count
add_action('wp_ajax_compprcl_get_product_count', 'compprcl_ajax_get_product_count');
function compprcl_ajax_get_product_count()
{
    check_ajax_referer('compprcl_cleanup_nonce', 'nonce');
    if (!current_user_can('manage_options')) {
        wp_send_json_error(array('message' => 'Permission denied'));
    }

    $count = compprcl_count_products();
    wp_send_json_success(array('count' => $count));
}

// AJAX: Scan images
add_action('wp_ajax_compprcl_scan_images', 'compprcl_ajax_scan_images');
function compprcl_ajax_scan_images()
{
    check_ajax_referer('compprcl_cleanup_nonce', 'nonce');
    if (!current_user_can('manage_options')) {
        wp_send_json_error(array('message' => 'Permission denied'));
    ...
    $results = compprcl_scan_orphaned_images_logic();
...
add_action('wp_ajax_compprcl_delete_images', 'compprcl_ajax_delete_images');
function compprcl_ajax_delete_images()
{
    check_ajax_referer('compprcl_cleanup_nonce', 'nonce');
    if (!current_user_can('manage_options')) {
        wp_send_json_error(array('message' => 'Permission denied'));
    ...
    $results = compprcl_delete_orphaned_images_logic();
...
add_action('wp_ajax_compprcl_delete_products_ajax', 'compprcl_ajax_delete_products');
function compprcl_ajax_delete_products()
{
    check_ajax_referer('compprcl_cleanup_nonce', 'nonce');
    if (!current_user_can('manage_options')) {
        wp_send_json_error(array('message' => 'Permission denied'));
    ...
    $results = compprcl_delete_all_products_logic($delete_attached, $delete_orphaned);
...
function compprcl_admin_page()
{
    ...
    ?>
    <div class="wrap">
        <div class="compprcl-wrapper">
            <div class="compprcl-header">
                <h1>Complete Product Cleaner for WooCommerce</h1>
                <p>Delete all products, variations, and clean up media library.</p>
            </div>

            <div class="compprcl-tabs">
                <div class="nav-tab-wrapper">
                    <button class="nav-tab compprcl-tab nav-tab-active" data-tab="products">
                        🗑️ Delete Products
                    </button>
                    <button class="nav-tab compprcl-tab" data-tab="images">
                        🖼️ Clean Images
                    </button>
                </div>

                <!-- Tab: Products -->
                <div id="compprcl-tab-products" class="compprcl-tab-content active">
                    <form id="compprcl-delete-form">
                        <div class="compprcl-card">
                            <h2>Delete All Products</h2>
                            <p>Select what else you want to delete along with products.</p>

                            <div class="compprcl-options">
                                <div class="compprcl-option">
                        ...
                        <div class="compprcl-danger-zone">
                            <h3>⚠️ Danger Zone</h3>
                            <p id="compprcl-delete-summary">This will delete <strong id="compprcl-product-count">...</strong> products and variations.</p>
                            <button type="button" class="button button-primary compprcl-button-delete">
                ...
                <!-- Tab: Images -->
                <div id="compprcl-tab-images" class="compprcl-tab-content">
                    <div class="compprcl-card">
                        <h2>Clean Orphaned Images</h2>
                        <p>Find and delete images that are not attached to any post, product, or page.</p>

                        <div style="margin-bottom: 20px;">
                            <button type="button" id="compprcl-scan-images" class="button button-secondary button-large">
                                🔍 Scan for Orphaned Images
                            </button>
                        </div>

                        <div id="compprcl-scan-results">
...
function compprcl_count_products()
{
    $count = 0;
...
function compprcl_scan_orphaned_images_logic()
{
    global $wpdb;
...
function compprcl_delete_orphaned_images_logic()
{
    $scan_results = compprcl_scan_orphaned_images_logic();
    $deleted_count = 0;
    $freed_space = 0;
...
function compprcl_delete_all_products_logic($delete_attached_images = false, $delete_orphaned_images = false)
{
    global $wpdb;
    ...
    try {
        ...
        if ($delete_orphaned_images) {
            $orphaned_results = compprcl_delete_orphaned_images_logic();
...
// Add settings link
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'compprcl_plugin_action_links');
function compprcl_plugin_action_links($links)
{
...
// Activation hook
register_activation_hook(__FILE__, 'compprcl_activation_notice');
function compprcl_activation_notice()
{
    set_transient('compprcl_activation_notice', true, 5);
}

add_action('admin_notices', 'compprcl_show_activation_notice');
function compprcl_show_activation_notice()
{
    if (get_transient('compprcl_activation_notice')) {
    ?>
...
<?php
        delete_transient('compprcl_activation_notice');
    }
}

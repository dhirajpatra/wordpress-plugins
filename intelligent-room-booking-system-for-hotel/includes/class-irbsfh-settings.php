...
class IRBSFH_Settings
{
    ...
    public static function get($key, $default = null)
    {
        ...
        $value = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT setting_value FROM {$wpdb->prefix}irbsfh_settings WHERE setting_key = %s",
                $key
            )
        );
    ...
    {
        global $wpdb;
        $table = $wpdb->prefix . 'irbsfh_settings';
        ...
        $existing = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT id FROM {$wpdb->prefix}irbsfh_settings WHERE setting_key = %s",
                $key
            )
        );
        ...
        if ($result) {
            wp_cache_delete($key, 'irbsfh_settings');
        }
    ...
    public static function set_defaults()
    {
        $defaults = array(
            ...
            'booking_title_default' => __('Room', 'intelligent-room-booking-system-for-hotel'),
            'email_user_confirmation_subject' => __('Booking Confirmed - {site_name}', 'intelligent-room-booking-system-for-hotel'),
            'email_user_confirmation_body' => self::get_default_user_confirmation_template(),
            'email_user_denial_subject' => __('Booking Request Declined - {site_name}', 'intelligent-room-booking-system-for-hotel'),
            'email_user_denial_body' => self::get_default_user_denial_template(),
            'email_admin_notification_subject' => __('New Booking Request - {site_name}', 'intelligent-room-booking-system-for-hotel'),

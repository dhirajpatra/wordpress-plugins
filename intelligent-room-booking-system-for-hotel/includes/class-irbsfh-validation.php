...
class IRBSFH_Validation
{
    ...
    public static function validate_booking_form($data)
    {
        ...
        if (empty($data['first_name'])) {
            $errors['first_name'] = __('First name is required.', 'intelligent-room-booking-system-for-hotel');
        } elseif (strlen($data['first_name']) > 100) {
            $errors['first_name'] = __('First name must be less than 100 characters.', 'intelligent-room-booking-system-for-hotel');
        }

        // Validate last name
        if (empty($data['last_name'])) {
            $errors['last_name'] = __('Last name is required.', 'intelligent-room-booking-system-for-hotel');
        } elseif (strlen($data['last_name']) > 100) {
            $errors['last_name'] = __('Last name must be less than 100 characters.', 'intelligent-room-booking-system-for-hotel');
        }

        // Validate email
        if (empty($data['email'])) {
            $errors['email'] = __('Email address is required.', 'intelligent-room-booking-system-for-hotel');
        } elseif (! is_email($data['email'])) {
            $errors['email'] = __('Please enter a valid email address.', 'intelligent-room-booking-system-for-hotel');
        }

        // Validate phone
        if (empty($data['phone'])) {
            $errors['phone'] = __('Phone number is required.', 'intelligent-room-booking-system-for-hotel');
        } elseif (! self::validate_phone($data['phone'])) {
            $errors['phone'] = __('Please enter a valid phone number.', 'intelligent-room-booking-system-for-hotel');
        }

        // Validate room ID
        if (empty($data['room_id']) || ! is_numeric($data['room_id'])) {
            $errors['room_id'] = __('Please select a valid room.', 'intelligent-room-booking-system-for-hotel');
        } else {
            $room = IRBSFH_Database::get_room(intval($data['room_id']));
            if (! $room) {
                $errors['room_id'] = __('Selected room does not exist.', 'intelligent-room-booking-system-for-hotel');
            }
        }

        // Validate booking date
        if (empty($data['booking_date'])) {
            $errors['booking_date'] = __('Booking date is required.', 'intelligent-room-booking-system-for-hotel');
        } else {
    ...
    public static function validate_booking_date($date)
    {
        ...
        if (! $date_obj || $date_obj->format('Y-m-d') !== $date) {
            return array(
                'valid' => false,
                'message' => __('Invalid date format.', 'intelligent-room-booking-system-for-hotel'),
            );
        }
        ...
        if ($date_obj < $today) {
            return array(
                'valid' => false,
                'message' => __('Cannot book dates in the past.', 'intelligent-room-booking-system-for-hotel'),
            );
        }
        ...
        if (in_array($day_of_week, $closed_days)) {
            ...
            return array(
                ...
                'message' => sprintf(
                    ...
                    __('Bookings are not available on %s.', 'intelligent-room-booking-system-for-hotel'),
    ...
    public static function can_user_book($user_id, $room_id)
    {
        ...
        if (! $room) {
            return array(
                'can_book' => false,
                'message' => __('Invalid room selected.', 'intelligent-room-booking-system-for-hotel'),
            );
        }

        // Check booking limit
        $max_bookings = isset($room->max_bookings_per_user) ? intval($room->max_bookings_per_user) : 3;
        $current_bookings = IRBSFH_Database::count_user_active_bookings($user_id, $room_id);
        ...
        if ($current_bookings >= $max_bookings) {
            return array(
                ...
                'message' => sprintf(
                    ...
                    __('You have reached the maximum of %d active bookings for this room.', 'intelligent-room-booking-system-for-hotel'),
                    $max_bookings
                ),
            );
        }
    ...
    {
        $bookings = IRBSFH_Database::get_room_bookings($room_id, $date);
        ...
        if (! empty($confirmed_bookings)) {
            return array(
                'available' => false,
                'message' => __('This room is already booked for the selected date.', 'intelligent-room-booking-system-for-hotel'),
            );
        }

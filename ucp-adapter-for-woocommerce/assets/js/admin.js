...
(function($) {
	...
	const UCPAdapterAdmin = {
		...
		regenerateAPIKey: function(e) {
			...
			$.ajax({
				url: ucpAdapterAdmin.ajaxUrl,
				method: 'POST',
				data: {
					action: 'ucp_regenerate_api_key',
					nonce: ucpAdapterAdmin.nonce
				}
			})
			.done((response) => {
				if (response.success) {
	...
	if (typeof wp !== 'undefined' && wp.ajax) {
		wp.ajax.post('ucp_regenerate_api_key', {
			nonce: ucpAdapterAdmin.nonce
		});
	}
 ...
 * 
 * add_action('wp_ajax_ucp_regenerate_api_key', 'ucp_ajax_regenerate_api_key');
 * 
 ...
 *     }
 *     
 *     $new_key = UCP_Security::generate_api_key();

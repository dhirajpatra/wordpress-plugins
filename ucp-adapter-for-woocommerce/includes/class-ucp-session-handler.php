 ...
 */
class UCP_Session_Handler
{

	/**
	 * Single instance
	 *
	 * @var UCP_Session_Handler
	 */
	...
	private function __construct()
	{
		...
		if (! wp_next_scheduled('ucp_cleanup_sessions')) {
			wp_schedule_event(time(), 'hourly', 'ucp_cleanup_sessions');
		}
		add_action('ucp_cleanup_sessions', array(__CLASS__, 'cleanup_expired_sessions'));
	}
	...
	{
		wp_cache_delete('ucp_recent_sessions', 'ucp_adapter');
	}
	...
	public function create_session($platform = 'default', $user_data = array())
	{
		...
		$table_name = $wpdb->prefix . 'ucp_sessions';
	...
	{
		$cached_session = wp_cache_get('ucp_session_' . $session_id, 'ucp_adapter');
		...
		$session = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}ucp_sessions WHERE session_id = %s AND expires > %d",
				$session_id,
				time()
			),
			ARRAY_A
		...
		wp_cache_set('ucp_session_' . $session_id, $session, 'ucp_adapter', 3600);
	...
	public function update_session($session_id, $data)
	{
		...
		$this->clean_session_cache();
		wp_cache_delete('ucp_session_' . $session_id, 'ucp_adapter');
	...
	{
		$cached_sessions = wp_cache_get('ucp_recent_sessions', 'ucp_adapter');
		...
		$sessions = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}ucp_sessions ORDER BY created_at DESC LIMIT %d",
				$limit
			),
			ARRAY_A
		);

		wp_cache_set('ucp_recent_sessions', $sessions, 'ucp_adapter', 3600);
	...
	{
		return 'ucp_' . bin2hex(random_bytes(16)) . '_' . time();
	}
	...
	public static function cleanup_expired_sessions()
	{
		...
		$wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->prefix}ucp_sessions WHERE expires < %d",
				time()
			)
		);

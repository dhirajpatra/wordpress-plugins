...
class UCP_REST_API
{
	...
	public static function check_api_permission($request)
	{
		...
		if (! UCP_Security::verify_request($request)) {
			return new WP_Error(
				'ucp_forbidden',
	...
	public static function handle_session($request)
	{
		...
		$session_handler = UCP_Session_Handler::get_instance();
		...
		do_action('ucp_session_created', $session_id, $platform, $user_data);
	...
	public static function handle_update($request)
	{
		...
		do_action('ucp_session_updated', $session_id, $action, $data);
	...
	public static function handle_complete($request)
	{
		...
		$result = apply_filters('ucp_before_session_complete', $completion_data, $session_id, $session);
		...
		do_action('ucp_session_completed', $session_id, $status, $metadata);
	...
	private static function process_update_action($session_id, $action, $data, $session)
	{
		...
		switch ($action) {
			...
			default:
				$current_data = apply_filters("ucp_update_action_{$action}", $current_data, $data, $session_id);
				break;
		}
